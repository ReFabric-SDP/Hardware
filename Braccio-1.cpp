...
void smoothMove(Servo component, int degree){
  int currDegree = component.read();
...
unsigned int _Braccio::begin(int soft_start_level) {
	...
	step_shoulder = 90;//40
	step_elbow = 90;//180
	step_wrist_ver = 90;//170
	step_wrist_rot = 90;//0
	step_gripper = 10;//73

	//For each step motor this set up the initial degree
	
        smoothMove(base, step_base);
        delay(2000);
  	//smoothMove(shoulder, step_shoulder);
        int curr_shoulder = shoulder.read();
        while (curr_shoulder != 90) {
            if (curr_shoulder < 90)
                curr_shoulder++;
            else
                curr_shoulder--;
            shoulder.write(curr_shoulder);
            delay(1000);
            Serial.println("shoulder delayed");
}
        delay(2000);
  	smoothMove(elbow, step_elbow);
        delay(2000);
  	smoothMove(wrist_ver, step_wrist_ver);
        delay(2000);
  	smoothMove(wrist_rot, step_wrist_rot);
        delay(2000);
  	smoothMove(gripper, step_gripper);
        delay(2000);
